...
export function generateLuhnValidCardNumber(prefix: string, length: number = 16): string {
  let attempts = 0
...
function generateLuhnValidCardNumberFallback(prefix: string, length: number = 16): string {
...
export function generateVirtualCard(
  ...
  const prefix = prefixes[cardBrand][Math.floor(Math.random() * prefixes[cardBrand].length)]
  ...
  const expiryYear = currentDate.getFullYear() + Math.floor(Math.random() * 3) + 1
...
export function generateDemoTransaction(cardId: string, userId: string): Omit<VirtualCardTransaction, 'id' | 'createdAt'> {
