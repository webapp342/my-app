...
export async function saveUserTransactions(
  ...
  const relevantTransactions = transactions
    .filter(tx => 
      tx.type === 'deposit' || 
      (tx.type === 'token_transfer' && tx.to.toLowerCase() === walletAddress.toLowerCase())
    )
    .slice(0, 50) // LIMIT: Only process last 50 transactions to save space

  console.log(`[BALANCE TRACKING] Processing ${relevantTransactions.length} relevant transactions (limited for efficiency)`)

  for (const transaction of relevantTransactions) {
    try {
      // Determine transaction type
      const transactionType = transaction.type === 'deposit' ? 'deposit' : 'token_in'
      ...
      const transactionData: Database['public']['Tables']['user_transactions']['Insert'] = {
        ...
        network: transaction.network === 'ethereum' ? 'ETHEREUM' : 'BSC_MAINNET',
      ...
      if (error) {
        if (error.code === '23505') { // Duplicate key error
          console.log(`[BALANCE TRACKING] Transaction ${transaction.hash} already exists, skipping`)
        } else {
          console.error(`[BALANCE TRACKING] Error saving transaction ${transaction.hash}:`, error)
          errors.push(`Transaction ${transaction.hash}: ${error.message}`)
        }
      } else {
        savedCount++
        console.log(`[BALANCE TRACKING] Saved transaction: ${transaction.hash} - ${amount} ${tokenSymbol}`)
      }

    } catch (error) {
      console.error(`[BALANCE TRACKING] Unexpected error processing transaction:`, error)
...
export async function getUserBalances(userId: string): Promise<UserBalance[]> {
  try {
    ...
    console.error('[BALANCE TRACKING] Unexpected error fetching balances:', error)
    return []
  }
}
...
export async function getUserTransactionHistory(
  ...
  try {
    ...
    if (error) {
      console.error('[BALANCE TRACKING] Error fetching transaction history:', error)
      return []
    }

    return data || []
  } catch (error) {
    console.error('[BALANCE TRACKING] Unexpected error fetching transaction history:', error)
    return []
  }
}
 ...
 */
export async function recalculateUserBalances(userId: string): Promise<boolean> {
  try {
    ...
    console.log(`[BALANCE TRACKING] Successfully recalculated balances for user ${userId}`)
    return true
  } catch (error) {
    console.error('[BALANCE TRACKING] Unexpected error recalculating balances:', error)
    return false
  }
}
