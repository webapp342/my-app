...
export async function fetchTokenBalances(address: string): Promise<TokenBalance[]> {
  console.log(`[PORTFOLIO DEBUG] Starting fetchTokenBalances for address: ${address}`)
  
  if (!BSCSCAN_API_KEY) {
    console.error('[PORTFOLIO DEBUG] BSCScan API key not found in environment variables')
    throw new Error('BSCScan API key not configured')
  }

  console.log(`[PORTFOLIO DEBUG] Using BSCSCAN_API_KEY: ${BSCSCAN_API_KEY.substring(0, 8)}...`)
  ...
  try {
    ...
    console.log(`[PORTFOLIO DEBUG] Making API call to: ${apiUrl.replace(BSCSCAN_API_KEY, 'API_KEY_HIDDEN')}`)
    
    const response = await fetch(apiUrl)
    console.log(`[PORTFOLIO DEBUG] API Response status: ${response.status}`)
    
    if (!response.ok) {
      console.error(`[PORTFOLIO DEBUG] BSCScan API HTTP error: ${response.status} ${response.statusText}`)
      throw new Error(`BSCScan API error: ${response.status}`)
    }

    const data = await response.json()
    console.log(`[PORTFOLIO DEBUG] API Response data status: ${data.status}, message: ${data.message}`)
    
    if (data.status !== '1') {
      console.log('[PORTFOLIO DEBUG] No token transactions found or API error:', data.message)
      return []
    }
    ...
    for (const token of uniqueTokens.values()) {
      try {
        console.log(`[PORTFOLIO DEBUG] Fetching balance for token: ${token.tokenSymbol} (${token.contractAddress})`)
        
        const balanceRaw = await fetchTokenBalanceRaw(address, token.contractAddress)
        console.log(`[PORTFOLIO DEBUG] Raw balance for ${token.tokenSymbol}: ${balanceRaw}`)
        ...
        if (parseFloat(balanceRaw) > 0) {
          ...
          console.log(`[PORTFOLIO DEBUG] Formatted balance for ${token.tokenSymbol}: ${balance} (decimals: ${decimals})`)
          
          const price = await fetchTokenPrice(token.tokenSymbol, 'BSC_MAINNET')
          const value = balance * price
          
          console.log(`[PORTFOLIO DEBUG] Price for ${token.tokenSymbol}: $${price}, Value: $${value}`)
        ...
        console.error(`[PORTFOLIO DEBUG] Error fetching balance for ${token.tokenSymbol}:`, error)
      }
    }

    console.log(`[PORTFOLIO DEBUG] Final token balances count: ${tokenBalances.length}`)
 ...
 */
async function fetchTokenBalanceRaw(address: string, contractAddress: string): Promise<string> {
  if (!BSCSCAN_API_KEY) {
    throw new Error('BSCScan API key not configured')
  }
  ...
  try {
    ...
    console.log(`[PORTFOLIO DEBUG] Token balance API call: ${apiUrl.replace(BSCSCAN_API_KEY, 'API_KEY_HIDDEN')}`)
    
    const response = await fetch(apiUrl)
    console.log(`[PORTFOLIO DEBUG] Token balance response status: ${response.status}`)

    if (!response.ok) {
      console.error(`[PORTFOLIO DEBUG] Token balance API error: ${response.status} ${response.statusText}`)
      throw new Error(`BSCScan API error: ${response.status}`)
    }
    ...
    if (data.status !== '1') {
      console.log(`[PORTFOLIO DEBUG] Token balance API returned status: ${data.status}, message: ${data.message}`)
      return '0'
    }
...
export async function getPortfolioData(address: string): Promise<PortfolioData> {
  try {
    console.log(`[PORTFOLIO DEBUG] Fetching complete portfolio for address: ${address}`)
    ...
    const { calculateUSDTValue, getNativeCurrencySymbol } = await import('./binance-price')
