...
function SendBNBContent() {
  ...
  const fetchUserBalance = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/get-user-balances?address=${address}&tokenSymbol=BNB`
      );
  ...
  const estimateGas = async () => {
    ...
    try {
      ...
      const totalCost = (
        parseFloat(amount) + parseFloat(data.estimatedFeeETH)
      ).toFixed(6);
      const estimatedFeeUSD = (
        parseFloat(data.estimatedFeeETH) * bnbPrice
      ).toFixed(2);
    ...
    try {
      ...
      if (data.error) {
        if (data.code === 'WALLET_NOT_FOUND') {
          throw new Error(
            'This wallet is not registered in our system. Please create or import a wallet first.'
          );
        }
        throw new Error(data.error);
      }
  ...
  const handleMaxAmount = () => {
    ...
    setError(
      `💡 MAX amount set to ${maxAmount.toFixed(6)} BNB. We automatically reserve 0.0005 BNB for gas fees to ensure your transaction completes successfully on the BSC blockchain.`
    );
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (numericValue > maxSendable) {
      ...
      setError(
        `⚠️ Maximum sendable amount is ${maxSendable.toFixed(6)} BNB. We reserve 0.0005 BNB for transaction gas fees to ensure your transaction can be processed successfully on the BSC network.`
      );
  ...
  const canProceed =
    ...
    isValidAddress &&
    parseFloat(amount) > 0 &&
    parseFloat(amount) <= getMaxSendableAmount();
              ...
              <div className='flex items-center space-x-3'>
                ...
                <div>
                  ...
                  <p className='text-xl font-bold text-gray-900'>
                    {isLoadingBalance
                      ? '...'
                      : `${parseFloat(userBalance).toFixed(6)} BNB`}
                  </p>
        ...
        <div className='bg-white rounded-2xl shadow-xl p-6'>
                ...
                <div className='space-y-1'>
                  ...
                  <p className='text-xs text-gray-400'>
                    ...
                    <span className='text-gray-500'>
                      {' '}
                      • Gas reserve: 0.0005 BNB for transaction fees
                    </span>
                  </p>
                </div>
              </div>
                ...
                <div
                  className={`border rounded-xl p-4 ${
                    error.includes('💡') || error.includes('MAX amount set')
                      ...
                      : error.includes('⚠️') ||
                          error.includes('Maximum sendable')
              ...
              <div className='bg-gray-50 rounded-xl p-4 space-y-4'>
                ...
                <div className='flex justify-between'>
                  ...
                  <span className='font-semibold'>
                    {transactionData.amount} BNB
                  </span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-gray-600'>Network Fee</span>
                  <span className='text-sm'>
                    ≈ ${transactionData.estimatedFeeUSD} USD
                  </span>
                </div>
              ...
              <div className='bg-blue-50 rounded-xl p-4'>
                ...
                <div className='space-y-2 text-sm'>
                  ...
                  <div className='flex justify-between'>
                    ...
                    <span className='text-blue-900'>
                      {ethers.formatUnits(transactionData.gasPrice, 'gwei')}{' '}
                      Gwei
                    </span>
                  </div>
                </div>
          ...
          {step === 'success' && (
            <div className='text-center space-y-6'>
              ...
              <p className='text-gray-600'>
                Your BNB has been sent successfully.
              </p>
