...
export async function POST(request: NextRequest) {
  try {
    const { fromAddress, toAddress, amount, gasLimit, gasPrice } =
      await request.json();
    ...
    if (walletError || !wallet) {
      ...
      return NextResponse.json(
        {
          error:
            'Wallet not found in database. Please create or import a wallet first.',
    ...
    try {
      ...
      console.log(
        'Private key decoded successfully, length:',
        privateKey.length
      );
    } catch (decodeError) {
    ...
    try {
      ...
      if (error.code === 'INSUFFICIENT_FUNDS') {
        ...
        return NextResponse.json(
          { error: 'Transaction failed: nonce expired' },
          { status: 400 }
        );
      } else if (error.code === 'REPLACEMENT_UNDERPRICED') {
        return NextResponse.json(
          { error: 'Transaction failed: gas price too low' },
          { status: 400 }
        );
      } else {
        return NextResponse.json(
    ...
    try {
      ...
      try {
        ...
        if (balanceError || !currentBalance) {
          console.warn(
            'Could not fetch current balance for update:',
            balanceError
          );
        } else {
          const newBalance = Math.max(
            0,
            parseFloat(currentBalance.balance) - parseFloat(totalDeductedBNB)
          );
      ...
      try {
        ...
        console.error(
          'Error updating balance for pending transaction:',
          balanceUpdateError
        );
      }
      ...
      return NextResponse.json({
        ...
        status: 'pending',
        message:
          'Transaction sent but confirmation failed. Check BSCScan for status.',
      });
    }
  } catch (error) {
