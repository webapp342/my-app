...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[PORTFOLIO API] BSCScan API Key available: ${process.env.BSCSCAN_API_KEY ? 'YES' : 'NO'}`)
    ...
    if (!isValidAddress(address)) {
      console.error(`[PORTFOLIO API] Error: Invalid address format: ${address}`)
      return NextResponse.json(
