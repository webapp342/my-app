...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!address && !username && !userId) {
      return NextResponse.json({ error: 'Address, username, or userId is required' }, { status: 400 })
    }
    ...
    if (tokenSymbol) {
      ...
      const singleBalance = balanceData as { balance: string; wallet_address: string } | null
      return NextResponse.json({
        success: true,
      ...
      const balanceArray = balanceData as { token_symbol: string; balance: string; wallet_address: string; user_id: string }[] | null
      
      // Create a complete list with all supported tokens (0 balance if not found)
      const completeBalances = SUPPORTED_TOKENS.map(token => {
        const existingBalance = balanceArray?.find(b => b.token_symbol === token)
        return {
          token_symbol: token,
