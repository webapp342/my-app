...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `[BSCScan TEST] API key available: ${apiKey.substring(0, 8)}...`
    );
    ...
    console.log(
      '[BSCScan TEST] Token tx response status:',
      tokenTxData.status,
      'message:',
      tokenTxData.message
    );
    console.log(
      '[BSCScan TEST] Token tx count:',
      tokenTxData.result ? tokenTxData.result.length : 0
    );
