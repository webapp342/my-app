...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    requestCounter++;
    console.log(
      `[PURCHASE] 🚀 Request #${requestCounter}: ${tokenQuantity} BBLIP for $${totalUsdCost}`
    );
    ...
    if (!/^\d{6}$/.test(transactionPassword)) {
      return NextResponse.json(
        { error: 'Transaction password must be 6 digits' },
        { status: 400 }
      );
    }
    ...
    if (!userData.transaction_password) {
      return NextResponse.json(
        {
          error:
            'Transaction password not set. Please set your password in wallet settings.',
        },
        { status: 400 }
      );
    }
    ...
    if (userData.transaction_password !== transactionPassword) {
      console.log(
        `[PURCHASE] ❌ Invalid transaction password for user ${userId}`
      );
      return NextResponse.json(
    ...
    console.log(
      `[PURCHASE] 💰 Current balances:`,
      balances?.map(b => `${b.token_symbol}: ${b.balance}`)
    );
    ...
    for (const token of uniqueTokens) {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/get-binance-price?symbol=${token}`
        );
    ...
    console.log(
      `[PURCHASE] 💵 Total available: $${totalAvailableUsd.toFixed(2)}`
    );
    console.log(
      `[PURCHASE] 📊 Token breakdown:`,
    ...
    if (totalAvailableUsd < totalUsdCost) {
      console.log(
        `[PURCHASE] ❌ Insufficient balance: need $${totalUsdCost}, have $${totalAvailableUsd.toFixed(2)}`
      );
      return NextResponse.json(
        {
    ...
    console.log(
      `[PURCHASE] 🎯 Asset priorities:`,
      priorities?.map(p => `${p.token_symbol} (Priority: ${p.priority_order})`)
    );
    ...
    const sortedTokens = Object.keys(tokenBalances).sort((a, b) => {
      const aPriority =
        priorities?.find(p => p.token_symbol === a && p.is_enabled)
          ?.priority_order || 999;
      const bPriority =
        priorities?.find(p => p.token_symbol === b && p.is_enabled)
          ?.priority_order || 999;
      return aPriority - bPriority;
    });

    console.log(
      `[PURCHASE] 🔄 Processing tokens in priority order:`,
      sortedTokens
    );
    ...
    for (const tokenSymbol of sortedTokens) {
      ...
      if (tokenPrice <= 0 || tokenData.total <= 0) {
        console.log(
          `[PURCHASE] ⏭️ Skipping ${tokenSymbol}: price=${tokenPrice}, balance=${tokenData.total}`
        );
        continue;
      }
      ...
      if (tokenToUse > 0) {
        ...
        console.log(
          `[PURCHASE] 💰 Using ${tokenToUse.toFixed(6)} ${tokenSymbol} @ $${tokenPrice.toFixed(2)} = $${usdValue.toFixed(2)}`
        );
      }
    }

    console.log(
      `[PURCHASE] 📋 Final payment plan:`,
      paymentPlan.map(p => `${p.requiredAmount.toFixed(6)} ${p.tokenSymbol}`)
    );
    ...
    if (masterTxError) {
      console.error(
        `[PURCHASE] ❌ Failed to record master transaction:`,
        masterTxError
      );
    ...
    console.log(
      `[PURCHASE] ✅ Created master transaction: ${masterPurchaseId}`
    );
    ...
    for (let i = 0; i < paymentPlan.length; i++) {
      ...
      console.log(
        `[PURCHASE] 💳 Processing ${payment.tokenSymbol}: ${remainingToDeduct.toFixed(6)} from ${payment.records.length} records`
      );
      ...
      for (const record of payment.records) {
        ...
        if (deductAmount > 0) {
          console.log(
            `[PURCHASE] 📉 Deducting ${deductAmount.toFixed(6)} from record ${record.id} (balance: ${currentBalance})`
          );
          ...
          if (updateError) {
            console.error(
              `[PURCHASE] ❌ Failed to update balance:`,
              updateError
            );
            throw new Error(
              `Failed to update balance for ${payment.tokenSymbol}`
            );
          }

          remainingToDeduct -= deductAmount;
          console.log(
            `[PURCHASE] ✅ Updated balance to ${newBalance.toFixed(6)}, remaining: ${remainingToDeduct.toFixed(6)}`
          );
        }
      }
      ...
      if (cardTxError) {
        console.error(
          `[PURCHASE] ❌ Failed to record virtual card transaction:`,
          cardTxError
        );
      ...
      console.log(
        `[PURCHASE] ✅ Recorded payment detail transaction: $${cardTxAmount.toFixed(2)} using ${payment.tokenSymbol}`
      );
    }
    ...
    if (existingBblip) {
      ...
      if (bblipError) {
        console.error(
          `[PURCHASE] ❌ Failed to update BBLIP balance:`,
          bblipError
        );
    ...
    console.log(
      `[PURCHASE] ✅ All transactions recorded under Purchase ID: ${masterPurchaseId}`
    );
    ...
    const { data: masterTransactions, error: masterTxFetchError } =
      await supabase
        .from('virtual_card_transactions')
        .select('amount')
        .eq('card_id', cardId)
    ...
    if (masterTxFetchError) {
      console.error(
        `[PURCHASE] ❌ Failed to fetch master transactions:`,
        masterTxFetchError
      );
    ...
    const calculatedTotalSpent = (masterTransactions || []).reduce(
    ...
    if (cardUpdateError) {
      console.error(
        `[PURCHASE] ❌ Failed to update virtual card:`,
        cardUpdateError
      );
    ...
    console.log(
      `[PURCHASE] ✅ Updated virtual card total_spent to: $${calculatedTotalSpent.toFixed(2)} (from ${masterTransactions?.length || 0} master transactions)`
    );
    ...
    console.log(
      `[PURCHASE] 🔍 FINAL BALANCES:`,
      finalBalances?.map(b => `${b.token_symbol}: ${b.balance} (ID: ${b.id})`)
    );

    // 10. SUCCESS RESPONSE
    console.log(`[PURCHASE] 🎉 Purchase completed successfully!`);
    console.log(
      `[PURCHASE] 📊 Summary: ${tokenQuantity} BBLIP for $${totalUsdCost}`
    );
