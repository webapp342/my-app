...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const result = await fetchTransactionHistory(address, network, 1, 100, startBlock)
      transactions = result.transactions
      total = result.total
      console.log(`[SYNC TRANSACTIONS] Fetched ${transactions.length} new transactions from blockchain (from block ${startBlock})`)
      
      // Early exit if no new transactions
      if (transactions.length === 0) {
        console.log(`[SYNC TRANSACTIONS] No new transactions found since block ${startBlock}`)
        return NextResponse.json({
          success: true,
    ...
    for (const tx of transactions) {
      try {
        ...
        if (tx.type === 'deposit' && userWalletAddress && tx.to.toLowerCase() === userWalletAddress.toLowerCase()) {
          transactionType = 'deposit'
        } else if (tx.type === 'token_transfer' && userWalletAddress && tx.to.toLowerCase() === userWalletAddress.toLowerCase()) {
          transactionType = 'token_in'
        }
        
        // Skip outgoing transactions (withdraw, token_out)
        if (!transactionType) {
          console.log(`[SYNC TRANSACTIONS] Skipping outgoing transaction: ${tx.hash}`)
          continue
        }
...
async function updateUserBalanceIncoming(userId: string, tx: Transaction, network: string, transactionType: string) {
  try {
    ...
    if (transactionType === 'deposit') {
      ...
      if (amount > 0) {
        console.log(`[SYNC TRANSACTIONS] Processing incoming native currency: ${tokenSymbol} amount: ${amount}`)
        await incrementUserBalance(userId, tokenSymbol, amount, network === 'bsc' ? 'BSC_MAINNET' : 'ETHEREUM', null)
      }
    }
    ...
    if (transactionType === 'token_in' && tx.tokenSymbol && tx.tokenAmount) {
      ...
      const tokenAddress = (tx as Transaction & { tokenAddress?: string }).tokenAddress || null
      
      if (amount > 0) {
        console.log(`[SYNC TRANSACTIONS] Processing incoming token transfer: ${tx.tokenSymbol} amount: ${amount} address: ${tokenAddress}`)
        await incrementUserBalance(userId, tx.tokenSymbol, amount, network === 'bsc' ? 'BSC_MAINNET' : 'ETHEREUM', tokenAddress)
      }
    }
...
async function incrementUserBalance(userId: string, tokenSymbol: string, amount: number, network: string, tokenAddress?: string | null) {
  ...
  if (existingBalance) {
    ...
    console.log(`[SYNC TRANSACTIONS] Incrementing ${tokenSymbol} balance: ${existingBalance.balance} + ${amount} = ${newBalance}`)
    ...
    console.log(`[SYNC TRANSACTIONS] Creating new ${tokenSymbol} balance: ${amount}`)
