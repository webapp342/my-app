...
export async function POST(request: NextRequest) {
  try {
    ...
    const tokenGroups: Record<string, { records: Array<{ id: string; balance: string; token_symbol: string }>, totalBalance: number }> = {}
    ...
    for (const [tokenSymbol, group] of Object.entries(tokenGroups)) {
      console.log(`[FIX BALANCES] Processing ${tokenSymbol}: ${group.records.length} records, total balance: ${group.totalBalance}`)

      if (group.records.length > 1) {
        // Multiple records - consolidate them
        const mainRecord = group.records[0]
        const duplicateIds = group.records.slice(1).map((r: { id: string }) => r.id)

        console.log(`[FIX BALANCES] Consolidating ${tokenSymbol}: keeping ${mainRecord.id}, deleting ${duplicateIds.length} duplicates`)
        ...
        if (updateError) {
          console.error(`[FIX BALANCES] Failed to update main record for ${tokenSymbol}:`, updateError)
          throw updateError
        }
        ...
        if (deleteError) {
          console.error(`[FIX BALANCES] Failed to delete duplicates for ${tokenSymbol}:`, deleteError)
          throw deleteError
        }
    ...
    console.log(`[FIX BALANCES] ✅ Cleanup completed. Final balances:`, finalBalances?.map(b => `${b.token_symbol}: ${b.balance}`))
