...
export function useBalanceTracking(userId: string | null, walletAddress: string | null) {
  ...
  const syncTransactions = useCallback(async () => {
    if (!userId || !walletAddress) {
      console.warn('[BALANCE TRACKING] Missing userId or walletAddress for sync')
      return
    }

    setState(prev => ({ ...prev, isSyncing: true, error: null, syncStats: null }))

    try {
      console.log(`[BALANCE TRACKING] Starting sync for user ${userId}, wallet ${walletAddress}`)
  ...
  useEffect(() => {
    if (userId && walletAddress && state.transactionCount === 0 && !state.isLoading && !state.isSyncing) {
      // Only auto-sync if user explicitly requests it (don't auto-sync on every page load)
      console.log('[BALANCE TRACKING] No transactions found. Click "Sync Transactions" to load data.')
    }
  }, [userId, walletAddress, state.transactionCount, state.isLoading, state.isSyncing])
