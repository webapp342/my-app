...
export default function Portfolio({ address, className = '' }: PortfolioProps) {
  ...
  const nativeBalance = portfolioData.nativeBalance || { symbol: 'BNB', balance: 0, value: 0 }
    ...
    <div className={`bg-white rounded-2xl shadow-xl overflow-hidden ${className}`}>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <p className="text-3xl font-bold">${formatNumber(netWorthUSD, 2)}</p>
          </div>

          {/* Net Worth BNB */}
          <div>
            <p className="text-blue-100 text-sm">Net Worth in BNB</p>
            <p className="text-2xl font-semibold">⚪ {formatNumber(netWorthBNB, 6)}</p>
          </div>

          {/* 24h Change */}
          <div>
            <p className="text-blue-100 text-sm">Total Balance Change (24H)</p>
            <p className={`text-xl font-semibold ${getChangeColor(totalChange24h)}`}>
        ...
        <div className="mt-6">
          ...
          <div className="w-full bg-blue-500 rounded-full h-2">
            <div 
              ...
              style={{ 
                width: netWorthUSD > 0 ? `${Math.min(100, (nativeBalance.value / netWorthUSD) * 100)}%` : '0%'
              }}
            ></div>
          </div>
          <div className="flex items-center mt-2 text-xs space-x-4">
            <div className="flex items-center">
              ...
              <span>BNB {netWorthUSD > 0 ? ((nativeBalance.value / netWorthUSD) * 100).toFixed(1) : '0'}%</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-blue-400 rounded-full mr-1"></div>
              <span>Others {netWorthUSD > 0 ? (100 - (nativeBalance.value / netWorthUSD) * 100).toFixed(1) : '0'}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 text-sm font-medium text-gray-600">Token</th>
                ...
                <th className="text-right py-3 text-sm font-medium text-gray-600">Change (24H)</th>
                <th className="text-right py-3 text-sm font-medium text-gray-600">Amount</th>
                <th className="text-right py-3 text-sm font-medium text-gray-600">Value</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              ...
              <tr className="hover:bg-gray-50">
                ...
                <td className="py-4 text-right text-sm font-medium">
                  ${nativeBalance.balance > 0 ? formatNumber(nativeBalance.value / nativeBalance.balance, 2) : '0.00'}
                </td>
              ...
              {tokens.map((token: TokenBalance) => (
                <tr key={token.contractAddress} className="hover:bg-gray-50">
                  <td className="py-4 flex items-center">
                    ...
                    <div>
                      <p className="font-medium">{token.tokenSymbol || 'Unknown'}</p>
                      <p className="text-xs text-gray-500">{token.tokenName || 'Unknown Token'}</p>
                    </div>
                  </td>
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-gray-500">
          ...
          <p>Last updated: {portfolioData.lastUpdated ? new Date(portfolioData.lastUpdated).toLocaleString() : 'Never'}</p>
        </div>
      </div>
    </div>
  )
